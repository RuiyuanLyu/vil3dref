...
def ground_eval_subset(gt_anno_list, det_anno_list, logger=None, prefix=''):
    ...
    for sample_idx in tqdm(range(num_samples)):
        ...
        target_scores = np.array(det_anno['target_scores_3d'])  # (num_query, )
        ...
        iou_mat = compute_ious(pred_bboxes, gt_bboxes) # 1/25 seconds on gpu (how much on cpu?)
...
def get_corners(box):
    ...
    corners = cal_corners(center, size, rotmat).reshape(-1, 8, 3)
    return corners
...
if __name__ == '__main__':
    for i in tqdm(range(1000)):
        num_preds = 20
        num_gts = 30
        centers = np.random.rand(num_preds, 3)
        sizes = np.random.rand(num_preds, 3) + 10
        euler = np.random.rand(num_preds, 3) - .5
        pred_boxes = np.concatenate([centers, sizes, euler], axis=1)

        centers = np.random.rand(num_gts, 3)
        sizes = np.random.rand(num_gts, 3) + 10
        euler = np.random.rand(num_gts, 3) - .5
        gt_boxes = np.concatenate([centers, sizes, euler], axis=1)

        matched_row_inds, matched_col_inds, costs = matcher(pred_boxes, gt_boxes, [iou_cost_fn])

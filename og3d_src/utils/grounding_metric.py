...
def ground_eval_subset(gt_anno_list, det_anno_list, logger=None, prefix=''):
    ...
    for sample_idx in tqdm(range(num_samples)):
        ...
        pred_bboxes = to_9dof_box(index_box(det_anno['bboxes_3d'], top_idxs))
        gt_bboxes = to_9dof_box(gt_anno['gt_bboxes_3d'])
...
def iou_cost_fn(pred_boxes, gt_boxes):
    ious = pred_boxes.overlaps(pred_boxes, gt_boxes).cpu().numpy()
